...
1. 拖入 **"代码"** 节点，连接在 **大模型节点1** 之后

2. 节点命名: `关键词分割`

3. **输入变量**:
   - 创建变量 `input_text`
   - 值设为: `{{大模型节点1.keyword_text}}`
...
#### 步骤 5.1: 批处理体 - Google Search 节点

在批处理体内部：

1. 拖入 **Google Search** 插件节点
...
#### 步骤 5.2: 批处理体 - 大模型节点2（信息提取）

**目标**: 依据关键词提示，提取网页关键信息

1. 拖入第二个 **"大模型"** 节点，连接在 **Google Search 节点** 之后

2. 节点命名: `信息提取`

3. **输入变量**:
   - 创建变量 `web_content`，值设为: `{{Google_Search节点.data}}`
...
### 步骤 6: 大模型节点3 - 内容整合与报告生成

**目标**: 整合所有初步报告，生成最终的技术分析报告

#### 配置步骤：

1. 拖入第三个 **"大模型"** 节点，连接在 **批处理节点** 之后
...
**结束节点** 自动存在，将 **大模型节点3** 的 `final_report` 输出连接到 **End** 节点，作为整个工作流的最终产出。
